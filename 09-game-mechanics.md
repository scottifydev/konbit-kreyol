...
1. **The co-op formula (`konbit.ts`).** Current `summitState` clears on `allLegsDone && total >= threshold` — a **raw sum with no per-leg floor** (`konbit.ts:65`). Replace with the fractional dual-condition **`positionState`**: legs scored as `L_i = h_i / b_i` over band-fixed budgets (6/8/10), take iff `L_A + L_B ≥ 1.30 AND min(L_A, L_B) ≥ 0.40 AND allLegsDone`. Rename `summited`→`taken`/`held`, `summitState`→`positionState`, `passRope`→`passTheWord`; keep `konbit.mon` (Mòn = the mountains) and `marronage()`. Update `types.ts` (`summited` field) and `06-engineering.md` §5 trap references.
2. **Gentle SRS demotion (`srs.ts`).** `review()` currently demotes on **every** miss (`st.box = Math.max(0, st.box - 1)`, `srs.ts:45`). Spec (and Law: non-punitive) wants **2-consecutive-miss** demotion. **Recommend:** add a two-consecutive-miss guard in `review()`. *(Gate: ratify the guard vs. keep single-miss — Scott.)*
3. **Prod-solid "mixed review" clause (`srs.ts`).** `mastery()` prod-solid checks 3-consecutive-correct spanning ≥7 days (`srs.ts:57–60`) but **does not** yet check the "≥1 in mixed review" clause the spec names. **Decide:** enforce the clause or relax the spec.
