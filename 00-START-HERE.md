...
| 2026-07-11 | **The climb metaphor is PURGED** (owner: "still climb reference. purge it all"). The pre-pivot heritage-fitness frame — rope-as-belay, "climbers"/`grenpe`, "summit," "climb," "Ann monte," "pass the rope," "the mountain waits for you to summit," `Mòn` as the mechanic brand, "two brothers, one rope" — is gone. Red-team ruling: the sport metaphor dies; the **mountains stay only as real Haitian history** — marronage (the maroons falling back to the mountains to fight another day = the non-punitive failure mechanic), « Dèyè mòn gen mòn », the real mountain battles/geography, `mòn` as taught vocabulary. Reframes: the co-op battle = **holding/taking the position** (was summit); the handoff = **pass the word** down the line (was pass the rope); the bond = **the wire / konbit** (« Men anpil, chay pa lou »). All purged Kreyòl drafts drop to `null` (English until Manman re-coins in the revolution frame). |
...
| 2026-07-10 / re-glossed 2026-07-11 | **Working title: KÒD LA** = **"the code"** — the cipher the enemy can't read (the *rope* reading is purged with the climb metaphor). Attested material, no coinage; pending Manman like every Kreyòl string. The boys' build shows **"THE CODE"** until she passes it. "Konbit Kreyòl" stays as the program/repo name. `kòd` remains in the pending U2 scope batch (`04` §1.1), now glossed toward *code*. |
...
- Naming: **the title — "KÒD LA" = "the code"** (confirm *kòd* reads as the cipher/code, the rope reading is dropped); the co-op-battle/relay's Kreyòl name and the new revolution-frame Kreyòl for the purged climb strings (relay, hold-the-position, pass-the-word, marronage/fall-back — all `null` awaiting her); the voice-dispatch mechanic's Kreyòl name (vwadyo is currently unlawful — see `04-laws.md` §Lint); the Cipher Office's kid-facing name; the capstone event's name.
